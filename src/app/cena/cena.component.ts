...
export class CenaComponent implements OnInit {
  ...
  ngSelectedCena(event: any) {
    this.selectedCena = event.target.value;
    this.obtenerAlimentos(Number(this.selectedCena));
  }
  ...
  almacenarTablaMenu() {
    if (this.alimentoNutrientesTabla.length != 0) {
      sessionStorage.setItem("arrayCenaNombre", JSON.stringify((this.nombreAlimentoArray)));
      sessionStorage.setItem("arrayCenaUnidad", JSON.stringify((this.tipoUnidadArray)));
