 ...
 *              se consumen durante la comida del proyecto Contenido Nutricio *
 *              Front-End.                                                    *
 *****************************************************************************/
...
export class ComidaComponent implements OnInit {
  ...
  ngSelectedComida(event: any) {
    this.selectedComida = event.target.value;
    this.obtenerAlimentos(Number(this.selectedComida));
  }
  ...
  almacenarTablaMenu() {
    if (this.alimentoNutrientesTabla.length != 0) {
      sessionStorage.setItem("arrayComidaNombre", JSON.stringify((this.nombreAlimentoArray)));
      sessionStorage.setItem("arrayComidaUnidad", JSON.stringify((this.tipoUnidadArray)));
