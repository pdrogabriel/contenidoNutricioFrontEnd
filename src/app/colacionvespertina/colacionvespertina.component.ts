...
@Component({
  selector: 'app-colacionvespertina',
  templateUrl: './colacionvespertina.component.html',
  styleUrls: ['./colacionvespertina.component.css']
})
export class ColacionvespertinaComponent implements OnInit {
  ...
  selectedColacionVespertina: string = '';
  ...
  ngSelectedColacionVespertina(event: any) {
    this.selectedColacionVespertina = event.target.value;
    this.obtenerAlimentos(Number(this.selectedColacionVespertina));
  }
  ...
  almacenarTablaMenu() {
    if (this.alimentoNutrientesTabla.length != 0) {
      sessionStorage.setItem("arrayColacionVespertinaNombre", JSON.stringify((this.nombreAlimentoArray)));
      sessionStorage.setItem("arrayColacionVespertinaUnidad", JSON.stringify((this.tipoUnidadArray)));
